...
typedef struct {
    int total_blocks;
    int coverage_count;
    u_int8_t blocks[0x60];
} coverage;

coverage* shmem;

void __sanitizer_cov_trace_pc_guard_init(u_int32_t *start,
                                                    u_int32_t *stop) {

    // Shared memory init
    key = 1234 + getpid() % 0x100;
    shmid = shmget(key, sizeof(coverage), 0666 | IPC_CREAT);
    shmem = (coverage *) shmat(shmid, 0, 0);

    static u_int64_t N;  // Counter for the guards.
    if (start == stop || *start) return;  // Initialize only once.
    for (u_int32_t *x = start; x < stop; x++)
        *x = ++N;  // Guards should start from 1.


    shmem->total_blocks = N;
    shmem->coverage_count = 0;
    memset(shmem->blocks, 0, sizeof(shmem->blocks));
...
void __sanitizer_cov_trace_pc_guard(u_int32_t *guard) {
    ...
    shmem->blocks[index / 8] |= (1 << (index % 8));
    shmem->coverage_count += 1;
    *guard = 0;
}

void __sanitizer_cov_trace_pc_guard_fini() {
